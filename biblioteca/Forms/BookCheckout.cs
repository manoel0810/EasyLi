...
namespace biblioteca
{
    public partial class BookCheckout : Form
    {
        ...
        {
            if (!RegisteredBooks)
                DatabaseController.DML(String.Format("insert into tb_livros values ('{0}', '{1}', '{2}')", Tombo.Text, MGlobais.ValidarString(Livro.Text), MGlobais.FormatarDataSQL(DateTime.Today.ToShortDateString())));
        }

        private void RegistrarUsuario()
        {
            if (!RegisteredUser)
                DatabaseController.DML($"insert into users values ('{UserCode}', '{MGlobais.ValidarString(NomeAluno.Text)}', '0')");
        }
        ...
        private void SaveClick(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrWhiteSpace(Email.Text) && MGlobais.ValidateEmail(Email.Text) && MGlobais.CheckSMTPConfiguration())
                biblioteca.Email.EnviarEmail(string.Format("Olá {0}. Notamos que você realizou a retirada do livro '{1}'. Você possue 8(oito) dias para efetuar a devolução.", NomeAluno.Text, Livro.Text), "EasyLi - Retirada de Livro", Email.Text);

            string vquery = @"INSERT INTO tb_dadosaluno (T_ALUNO, T_LIVRO, T_STATUS, T_DATA, T_DATAP, T_TURMA, T_MATRICULA, T_TOMBO, T_EMAIL) VALUES ('" + MGlobais.ValidarString(NomeAluno.Text) + "', '" + MGlobais.ValidarString(Livro.Text) + "', '" + (int)InitialBookState + "', '" + MGlobais.FormatarDataSQL(Data.Text) + "', '" + MGlobais.FormatarDataSQL(Data.Text) + "', '" + Turma.Text + "', '" + UserCode + "', '" + Tombo.Text + "', '" + Email.Text + "')";
        ...
        private void MatriculaLeave(object sender, EventArgs e)
        {
            ...
            {
                RegisteredUser = true;
                NomeAluno.Text = Student.Rows[0].Field<string>("user_name");
