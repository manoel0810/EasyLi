...
namespace biblioteca
{
    public partial class BookCheckout : Form
    {
        ...
        {
            if (!RegisteredUser)
                DatabaseController.DML($"insert into users values ('{UserCode}', '{MGlobais.ValidarString(NomeUsuario.Text)}', '0')");
        }
        ...
        private void SaveClick(object sender, EventArgs e)
        {
            ...
            string vquery = @"INSERT INTO tb_dadosaluno (T_ALUNO, T_LIVRO, T_STATUS, T_DATA, T_DATAP, T_TURMA, T_MATRICULA, T_TOMBO, T_EMAIL) VALUES ('" + MGlobais.ValidarString(NomeUsuario.Text) + "', '" + MGlobais.ValidarString(Livro.Text) + "', '" + (int)InitialBookState + "', '" + MGlobais.FormatarDataSQL(Data.Text) + "', '" + MGlobais.FormatarDataSQL(Data.Text) + "', '" + Turma.Text + "', '" + UserCode + "', '" + Tombo.Text + "', '" + Email.Text + "')";
